...
export default class WalletTransfer extends Component {
  ...
  walletTransfer = async () => {
    ...
    fetch('https://swift2pay.com/account/api/request?action=resolveWalletAccount&recipient='+this.state.userID, {
      method: 'GET',
    })
    ...
    fetch('https://swift2pay.com/account/api/request?action=walletTransfer&recipientID='this.state.userID'&userID='+grabUserId+'&amount='+this.state.amount+'&apiKey=JFJHFJJ38388739949HFGDJ', {
      method: 'GET',
    })
    .then(response => response.json())
    .then((json) => {
